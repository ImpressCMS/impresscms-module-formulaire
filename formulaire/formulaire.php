...
include 'header.php';
include_once XOOPS_ROOT_PATH.'/libraries/phpmailer/class.phpmailer.php';

###############################################################################
## Changed Number: 80020141 - GibaPhp
## Included verification system ImpressCMS
## Kept previous code for the case to use Framework
## Messages were added temporarily until the official approval of the developer Philou.
## Only the work is being checked with the class Captcha system impressCMS
###############################################################################
...
if (defined('ICMS_VERSION_NAME')) {
	...
	if (file_exists (XOOPS_ROOT_PATH."/class/captcha/formcaptcha.php")) {
...
$destname = '';
global $xoopsDB, $myts, $xoopsUser, $xoopsModule, $xoopsTpl, $xoopsConfig, $destname, $allowed_mimetypes, $permittedtypes, $xoopsModuleConfig;
...
if( empty($_POST['submit']) ) {
    ...
    if ($elements) {
		...
		if (defined('ICMS_VERSION_NAME')) {
			// Captcha Hack
			if ( class_exists( 'IcmsFormCaptcha' ) && $xoopsModuleConfig['captcha'] == 1 ) {
	...
	if (defined('ICMS_VERSION_NAME')) {
		// Captcha Hack 
		// Verify entered code 
		if ( class_exists( 'IcmsCaptcha' ) && $xoopsModuleConfig['captcha'] == 1 ) { 
			if ( @include_once ICMS_ROOT_PATH . '/class/captcha/captcha.php' ) { 
				...
				if ( ! $icmsCaptcha -> verify( true ) ) { 
					...
					redirect_header( $redirect, 2, $icmsCaptcha -> getMessage() ); 
				}	 
			} 
		} 
		// Captcha Hack
		...
		if (@include_once ICMS_ROOT_PATH . "/class/captcha/captcha.php") {
			...
			if (! $xoopsCaptcha->verify() ) {
				...
				redirect_header( $redirect , 2, _CAPTCHA_INVALID_CODE ) ;
				exit();
			}
		} elseif(!empty($framework)) {
...
foreach ($_POST["xoops_upload_file"] as $indiceUpload => $fichier)
{
	if( !empty( $fichier ) || $fichier != "") {
		...
		// teste si aucun fichier n'a �t� joint
		...
		// teste si le fichier a �t� upload� dans le r�pertoire temporaire:
